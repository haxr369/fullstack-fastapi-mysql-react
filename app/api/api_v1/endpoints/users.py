...
from APIs.schemas import user_sch
from APIs.api import deps
from APIs.core.config import settings
from APIs.utils import send_new_account_email
...
@router.put("/me", response_model= user_sch.User)
def update_user_me(
    *,
    db: Session = Depends(deps.get_db),
    ...
    user_in = user_sch.UserUpdate(**current_user_data)
...
@router.get("/me", response_model= user_sch.User)
def read_user_me(
    db: Session = Depends(deps.get_db),
    current_user: user.User = Depends(deps.get_current_active_user),
) -> Any:
...
def create_user_open(
    *,
    ...
    user_in = user_sch.UserCreate(password=password, email=email, full_name=full_name)
    user = crud.user.create(db, obj_in=user_in)
    return user
